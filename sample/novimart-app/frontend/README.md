# NoviMart — Frontend (React + TypeScript + Vite)

The customer-facing SPA for NoviMart retail commerce. Pairs with the .NET 10
BFF API in `../api`.
...
VITE_AZURE_API_SCOPE=api://novimart-api/access_as_user
...
Per `.specfleet/policies/pci.md`, the SPA must redirect to a tokenising payment
...
img-src 'self' data: https://*.novimart.example;
...
- Colour contrast targets WCAG 2.1 AA (novimart-blue on white passes).
