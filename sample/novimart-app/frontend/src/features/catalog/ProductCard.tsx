...
export function ProductCard({ product }: Props) {
  ...
  return (
    <article className="flex flex-col rounded border border-gray-200 p-4">
      <Link to={`/products/${product.productId}`} className="text-base font-semibold text-novimart-blue">
        {product.name}
      </Link>
      <p className="mt-1 line-clamp-2 text-sm text-novimart-gray">{product.description}</p>
