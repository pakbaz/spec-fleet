# NoviMart E-Commerce — Infrastructure (Bicep + azd)

This directory provisions the full Azure footprint for the NoviMart
retail commerce sample. Everything is composed from `infra/main.bicep`
...
flowchart LR
  ...
  subgraph rg[rg-novimart-&lt;env&gt;-&lt;region&gt;]
    direction LR
...
2. `azd provision` deploys `main.bicep` at subscription scope, which:
   - creates `rg-novimart-${AZURE_ENV_NAME}-${AZURE_LOCATION}`
...
Every other control listed in `.specfleet/instruction.md` and `.specfleet/policies/`
