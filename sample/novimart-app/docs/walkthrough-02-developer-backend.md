...
> **Audience:** A .NET backend engineer joining the NoviMart team and picking up their first
> story under the SpecFleet.
>
...
>
> **What you will learn:** how `specfleet plan` decomposes the goal into role/subagent tasks, how each
...
3. [Step 1 — `specfleet plan`](#step-1--specfleet-plan-implement-product-search-story-1)
4. [Step 2 — Review the plan (human gate)](#step-2--review-the-plan-human-gate)
5. [Step 3 — `specfleet run --task=backend-products-endpoint`](#step-3--specfleet-run---taskbackend-products-endpoint)
6. [Step 4 — `specfleet run --task=tests-unit-product-search`](#step-4--specfleet-run---tasktests-unit-product-search)
7. [Step 5 — `specfleet run --task=tests-integration-products-endpoint`](#step-5--specfleet-run---tasktests-integration-products-endpoint)
8. [Step 6 — `specfleet review`](#step-6--specfleet-review)
9. [Step 7 — Run locally](#step-7--run-locally)
10. [Step 8 — Open the PR](#step-8--open-the-pr)
11. [What broke and how SpecFleet helped](#11-what-broke-and-how-specfleet-helped)
...
```bash
# Repo
git clone https://github.com/novimart/novimart-app.git
cd novimart-app

# Toolchain
dotnet --version          # expect 10.0.* (preview or GA)
node  --version           # expect v20+ (only needed for the SpecFleet CLI itself)
which specfleet                 # ensure the specfleet CLI is on PATH
...
> **Note:** SpecFleet itself is a Node 20+ tool that *orchestrates* the .NET work — you don't need
...
Verify your `.specfleet/` workspace is healthy before doing anything else:
...
> **What just happened?** A product manager wrote that issue. SpecFleet does not own backlog
> grooming; humans still decide *what* gets built. SpecFleet owns *how* — it decomposes the goal,
> dispatches role/subagents, and enforces every rule from `.specfleet/instruction.md` along the way.

---

## Step 1 — `specfleet plan "implement product search story #1"`

```bash
$ specfleet plan "implement product search story #1"
⠋ Planning: implement product search story #1
✔ Plan written to .specfleet/plans/2025-01-22T14-32-08-114Z.md
```

Behind the scenes the **orchestrator agent** booted, loaded `.specfleet/instruction.md` and
`.specfleet/project.md` into its context window, queried the codebase index for the symbols
...
> 🤖 **Simulated agent transcript** — produced from charter behavior; `specfleet plan` would emit
> equivalent narrative output when run with `SPECFLEET_DEBUG=1`.

```text
[orchestrator] cold start. token budget = 100000, used = 4127.
[orchestrator] reading .specfleet/instruction.md (4.1 KB) … rules cached.
[orchestrator] reading .specfleet/project.md (5.6 KB) … stack confirmed: minimal-api + cosmos.
[orchestrator] indexing references in goal: "product search"
  → hits in:
    backend/src/NoviMart.Domain/Services/ProductSearchSpecification.cs   (already exists, public Apply())
    backend/src/NoviMart.Domain/Repositories/Repositories.cs              (IProductRepository.SearchAsync)
    backend/src/NoviMart.Infrastructure/Cosmos/Repositories/ProductRepository.cs (Cosmos impl exists)
    backend/src/NoviMart.Contracts/V1/Catalog.cs                          (ProductSearchRequest exists, response NOT yet defined)
    backend/src/NoviMart.Api/NoviMart.Api.csproj                       (host project — empty of endpoints)
...
The plan written to `.specfleet/plans/2025-01-22T14-32-08-114Z.md` looks like this (abridged):
...
- id: contracts-add-search-response
  ...
  brief: |
    ...
    in NoviMart.Contracts.V1.Catalog. Reuse existing ProductSummaryDto.
    Add a static mapping from NoviMart.Domain.Services.ProductSearchResult to the new response.
...
- id: backend-products-endpoint
  ...
  brief: |
    Create backend/src/NoviMart.Api/Endpoints/ProductsEndpoint.cs exposing
...
- id: tests-unit-product-search
  ...
  brief: |
    Author tests/NoviMart.UnitTests/Domain/Services/ProductSearchSpecificationTests.cs
...
- id: tests-integration-products-endpoint
  ...
  brief: |
    Author tests/NoviMart.IntegrationTests/ProductsEndpointTests.cs. Use
...
> charter caps below it and SpecFleet aborts a task that would exceed its cap.
...
session: 9c2e1b3a-…
state:   plan_ready
plan:    .specfleet/plans/2025-01-22T14-32-08-114Z.md
tasks:   7 ready, 0 blocked, 0 in-progress, 0 done

✋ Review the plan before execution. Run `specfleet run --all` to proceed,
   or edit the plan file and run `specfleet run --task <id>` for a smaller slice.
```

A human (you) opens the plan, reads it line by line, and either:

1. **Approves** the plan as-is by executing it:

   ```bash
   $ specfleet run --all
   ▸ contracts-add-search-response → dev/backend
   ```

2. **Edits** the plan — for example, adding a `frontend-search-bar` task — then runs one task:

   ```bash
   $ vim .specfleet/plans/2025-01-22T14-32-08-114Z.md
   $ specfleet run --task frontend-search-bar
   ▸ frontend-search-bar → dev/frontend
   ```
...
## Step 3 — `specfleet run --task=backend-products-endpoint`

```bash
$ specfleet run --task=backend-products-endpoint
...
[dev/backend] loading allowlist files:
  - .specfleet/charters/dev.charter.md                                          (438 t)
  - .specfleet/charters/subagents/dev/backend.charter.md                        (262 t)
  - .specfleet/instruction.md                                                  (4127 t)
  - backend/src/NoviMart.Domain/Services/ProductSearchSpecification.cs (1184 t)
  - backend/src/NoviMart.Domain/Repositories/Repositories.cs            (387 t)
  - backend/src/NoviMart.Contracts/V1/Catalog.cs                       (1342 t)
  - backend/src/NoviMart.Api/NoviMart.Api.csproj                     (251 t)
  - backend/src/NoviMart.Api/Program.cs                                 (612 t)
  TOTAL                                                                   8603 t  (in budget)
[dev/backend] brief acknowledged. drafting Endpoints/ProductsEndpoint.cs.
[hooks/onPreToolUse] write-target: backend/src/NoviMart.Api/Endpoints/ProductsEndpoint.cs
[hooks/onPreToolUse] subagent allowlist permits writes under NoviMart.Api/**.  → ALLOW
...
The agent wrote `backend/src/NoviMart.Api/Endpoints/ProductsEndpoint.cs`:

```csharp
// Copyright (c) NoviMart. Licensed under the MIT License.
...
public static class ProductsEndpoint
{
    ...
    public static IEndpointRouteBuilder MapProducts(this IEndpointRouteBuilder app)
    {
        ...
        //   Catalog browse is a public marketing surface. NoviMart explicitly allows
...
```csharp
// backend/src/NoviMart.Contracts/V1/Catalog.cs (appended)
...
```csharp
// backend/src/NoviMart.Api/Endpoints/ProductSearchMapper.cs
...
`onPreToolUse` is a JS function loaded from `.specfleet/policies/hooks.ts` (compiled at startup).
...
// resolved policy snapshot
{
  "agent": "dev/backend",
  "writePathAllow": [
    "backend/src/NoviMart.Api/**",
    "backend/src/NoviMart.Contracts/**",
    "backend/src/NoviMart.Infrastructure/**",
    "backend/src/NoviMart.Domain/**"
  ],
  "writePathDeny": [".specfleet/instruction.md", ".specfleet/policies/**", "**/.env*"],
  "secretScan": "block-on-match"
}
```

If the agent had attempted to `Edit` `.specfleet/instruction.md`, the hook would have returned
...
```jsonl
{"ts":"2025-01-22T14:34:11.022Z","sid":"9c2e1b3a","agent":"dev/backend","tool":"Edit","path":"backend/src/NoviMart.Api/Endpoints/ProductsEndpoint.cs","sha256":"5f0c…d3","bytes":2148,"hook":"onPreToolUse:allow","redactedSecrets":0}
{"ts":"2025-01-22T14:34:11.611Z","sid":"9c2e1b3a","agent":"dev/backend","tool":"Edit","path":"backend/src/NoviMart.Api/Endpoints/ProductSearchMapper.cs","sha256":"7a91…22","bytes":1063,"hook":"onPreToolUse:allow","redactedSecrets":0}
{"ts":"2025-01-22T14:34:11.984Z","sid":"9c2e1b3a","agent":"dev/backend","tool":"Edit","path":"backend/src/NoviMart.Contracts/V1/Catalog.cs","sha256":"e2d4…b1","bytes":372,"hook":"onPreToolUse:allow","redactedSecrets":0}
```

Stream the audit live with `specfleet log --tail --agent=dev/backend`.

---

## Step 4 — `specfleet run --task=tests-unit-product-search`

```bash
$ specfleet run --task=tests-unit-product-search
...
[test/unit] loading:
  - .specfleet/charters/test.charter.md                                          (361 t)
  - .specfleet/charters/subagents/test/unit.charter.md                           (188 t)
  - backend/src/NoviMart.Domain/Services/ProductSearchSpecification.cs (1184 t)
  - backend/src/NoviMart.Domain/Entities/Catalog.cs                     (892 t)
  - backend/tests/NoviMart.UnitTests/NoviMart.UnitTests.csproj       (197 t)
  TOTAL                                                                   2822 t (in budget)
[test/unit] writing tests/NoviMart.UnitTests/Domain/Services/ProductSearchSpecificationTests.cs
[hooks/onPreToolUse] write under NoviMart.UnitTests/** → ALLOW
```
...
public class ProductSearchSpecificationTests
{
    ...
    {
        Make("p1", "shoes",  "NoviMart Runner",   tags: ["sport", "running"]),
        Make("p2", "shoes",  "NoviMart Loafer",   description: "leather loafer"),
        Make("p3", "bags",   "NoviMart Tote",     tags: ["leather"]),
        Make("p4", "bags",   "NoviMart Backpack", isActive: false), // soft-deleted
        Make("p5", "shirts", "NoviMart Polo",     tags: ["cotton"]),
    };
    ...
    public void Apply_NoFilter_ReturnsAllActive_OrderedByName()
    {
        ...
        r.Items.Select(p => p.Name).Should().ContainInOrder(
            "NoviMart Loafer", "NoviMart Polo", "NoviMart Runner", "NoviMart Tote");
    }
...
```text
$ dotnet test backend/tests/NoviMart.UnitTests --collect:"XPlat Code Coverage"
…
+--------------------------------+--------+--------+-------+
| Module                         | Line   | Branch | Method|
+--------------------------------+--------+--------+-------+
| NoviMart.Domain.Services    | 95.5%  | 91.7%  | 100%  |
+--------------------------------+--------+--------+-------+
...
## Step 5 — `specfleet run --task=tests-integration-products-endpoint`

```bash
$ specfleet run --task=tests-integration-products-endpoint
...
namespace NoviMart.IntegrationTests;
            ...
            b.ConfigureTestServices(services =>
            {
                ...
                fake.SearchAsync(default, default, default, default, default!)
                    .ReturnsForAnyArgs(_ => Task.FromResult(new ProductSearchResult(
                        Items: new[]
                        {
                            new Product(new ProductId("p1"), "shoes", "NoviMart Runner",
...
> (see `NoviMart.IntegrationTests.csproj`), so no new dependencies are added. SpecFleet would
...
[architect/solid] reading diff (4 files, +312 -0).
  ...
  → recorded in .specfleet/decisions.md as ARCH-NIT-001.
...
The Architect's nit is recorded in `.specfleet/decisions.md`; you can either fix it now or
...
```bash
# Build everything
$ dotnet build backend/NoviMart.sln -c Release
Build succeeded.
    0 Warning(s)
    0 Error(s)

# Run all tests
$ dotnet test backend/NoviMart.sln --collect:"XPlat Code Coverage"
…
...
Use the GitHub PR template populated by `specfleet review` (the file is written to
`.specfleet/checkpoints/<sid>/pr.md` whenever a session enters the `ready_for_pr` state):

```markdown
## Story #1 — Catalog: search by category & free-text

**Plan:** [`.specfleet/plans/2025-01-22T14-32-08-114Z.md`](../.specfleet/plans/2025-01-22T14-32-08-114Z.md)

### Diff summary
- **+** `backend/src/NoviMart.Api/Endpoints/ProductsEndpoint.cs` (64 LoC)
- **+** `backend/src/NoviMart.Api/Endpoints/ProductSearchMapper.cs` (32 LoC)
- **±** `backend/src/NoviMart.Api/Program.cs` (+3 LoC) — wires `MapProducts()` and DI
- **±** `backend/src/NoviMart.Contracts/V1/Catalog.cs` (+12 LoC) — `ProductSearchResponse`
- **+** `backend/tests/NoviMart.UnitTests/Domain/Services/ProductSearchSpecificationTests.cs` (124 LoC)
- **+** `backend/tests/NoviMart.IntegrationTests/ProductsEndpointTests.cs` (78 LoC)
...
### Audit summary (excerpt from `.specfleet/audit/9c2e1b3a.jsonl`)
...
### How to test locally
```bash
dotnet test backend/NoviMart.sln
ASPNETCORE_ENVIRONMENT=Development dotnet run --project backend/src/NoviMart.Api
curl 'http://localhost:5080/api/v1/products?categoryId=shoes&q=runner'
```
```

> **Note:** `specfleet pr` is on the roadmap (it would shell out to `gh pr create --body-file=…`).
> Today the CLI prints the PR body and you paste it into `gh`.

---

## 11. What broke and how SpecFleet helped

### Incident A — agent tried to add Newtonsoft

`dev/backend` initially proposed adding `<PackageReference Include="Newtonsoft.Json" />`
to `NoviMart.Api.csproj` to deserialise the request manually. The orchestrator's
pre-commit policy resolver flagged it:

```text
[hooks/onPreToolUse] write to backend/src/NoviMart.Api/NoviMart.Api.csproj
...
`compliance/gdpr` blocked the diff during `specfleet review`:
...
> agents stay focused, not a budget you have to optimise. SpecFleet deliberately spends more
...
```bash
specfleet plan "<goal>"               # decompose goal into role/subagent tasks
specfleet run --task=<id>             # run one task
specfleet run --all                   # run every ready task in topo order
specfleet review                      # architect + compliance + test re-review
specfleet status                      # session state, open gates
specfleet log --tail --agent=<n>      # follow audit events live
specfleet check                       # validate .specfleet/ integrity
```

For the full charter list see `.specfleet/charters/`. For policy bodies see `.specfleet/policies/`.
For the immutable engineering standards that bind every agent, see
[`.specfleet/instruction.md`](../.specfleet/instruction.md).
