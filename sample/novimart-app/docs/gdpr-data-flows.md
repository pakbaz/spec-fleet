# GDPR Data Flows — NoviMart

> **Scope:** This document describes how personal data flows through the NoviMart retail commerce
> sample, what legal basis applies, and how data subject rights (DSRs) are honoured. It is the
> reference document for the `compliance/gdpr` SpecFleet subagent.
>
...
| 6 | Cart contents (browser) | productId, quantity, version | localStorage `novimart.cart.v1` | Browser-managed | Until user clears or signs in |
| 7 | Telemetry | url path, status code, latency | App Insights | AES-256 (Microsoft-managed) | 90 days |
| 8 | Audit log | actor, action, target, timestamp | `.specfleet/audit/` (file) → Log Analytics in prod | AES-256 / TLS 1.2+ | 1 year |
...
> story. The SpecFleet `compliance/pci` subagent reviews every diff against this rule.
...
truncated to /24 by an `ITelemetryProcessor` — see `NoviMart.Infrastructure/Logging/
PiiRedactingTelemetryProcessor.cs`).

### 2.2 Sign-in (Entra External ID)

```
[Browser]  ──MSAL.js──▶  [Entra External ID tenant]  (no NoviMart database involved)
                                  │
                                  ▼
...
NoviMart **never** sees the password. Entra External ID is the identity provider of record;
...
[Browser]  ──HTTPS──▶  [API: POST /orders]
                              ...
                              └──▶ [PaymentProvider stub]  ──would call──▶  [Adyen / Stripe / Worldpay]
                                                                            (out of PCI scope for NoviMart)
```

The real provider returns a tokenised reference (`paymentToken`) which is stored on the order;
no PAN ever touches NoviMart infrastructure. See `pci-scope-boundary.md`.
...
[SpecFleet runtime]  ──JSONL──▶  .specfleet/audit/<date>_<session>.jsonl  ──in CI──▶  Log Analytics
                              │
...
| Telemetry / fraud prevention | **Legitimate interests** (Art. 6(1)(f)) | LIA documented in `.specfleet/policies/gdpr.md` |
...
`NoviMart.Infrastructure/Logging/PiiRedactingEnricher.cs` (referenced by `Program.cs` via
...
The SpecFleet `compliance/gdpr.charter.md` requires this table to be kept current; PRs that add
...
1. SRE on-call invokes the runbook in `.specfleet/policies/gdpr.md` § "Breach response".
2. SpecFleet captures a forensic snapshot of audit logs from the affected window.
...
## 8. SpecFleet subagent enforcement summary

The `compliance/gdpr` subagent (`.specfleet/charters/subagents/compliance/gdpr.charter.md`) enforces
...
- NoviMart policy file: `.specfleet/policies/gdpr.md`
