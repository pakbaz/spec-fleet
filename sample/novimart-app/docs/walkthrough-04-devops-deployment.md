# Walkthrough 04 — DevOps: Deploy NoviMart to Azure with `azd up`

> **Audience:** A platform / DevOps engineer responsible for shipping the NoviMart sample to
...
7. [Step 5 — Run the SpecFleet deployment compliance gate](#step-5--run-the-specfleet-deployment-compliance-gate)
...
az ad app create --display-name "NoviMart Storefront" \
...
az ad app create --display-name "NoviMart API" \
  --identifier-uris api://novimart \
...
azd env set AZURE_CIAM_AUDIENCE     "api://novimart"
...
Verify SpecFleet is still happy:
...
three SpecFleet-mandated checks **before** any cloud resource is created:

```bash
# 1) Validate Bicep with both `bicep build` and `az deployment what-if`
bicep build infra/main.bicep --outfile /tmp/main.json

# 2) Run the SpecFleet DevSecOps subagent against the planned diff
specfleet review --scope=infra --strict
...
If `specfleet review --scope=infra --strict` exits non-zero, `azd up` aborts.
...
Outputs:
  ...
  AZURE_STATIC_WEB_APP_URL     = https://novimart-web.azurestaticapps.net
  AZURE_COSMOS_ENDPOINT        = https://novimart-cosmos-xt5n.documents.azure.com:443/
  AZURE_KEY_VAULT_URI          = https://novimart-kv-xt5n.vault.azure.net/
...
## Step 5 — Run the SpecFleet deployment compliance gate

```bash
specfleet review --scope=deploy --env=novimart-dev
```
...
The `INFO`/`WARN` items are recorded into `.specfleet/audit/<date>_deploy_review.jsonl` for the
...
1. Creates a workforce app registration `novimart-cicd`.
...
|---------------|----------------------|-----------|
| `ci.yml`      | PR + push            | `dotnet build/test` (90% gate), `npm test`, `specfleet config validate` |
| `cd.yml`      | push to `main`       | `azd provision && azd deploy` to the dev environment |
| `security.yml`| PR + nightly         | CodeQL, Dependabot review, secret scan, `specfleet review --scope=code` |
...
az keyvault list-deleted --query "[?name=='novimart-kv-xt5n']"    # → []
```
...
```
Resource group: rg-novimart-<env>-<region>-<token>
  ├─ Container Apps environment        cae-novimart-<token>
  │    └─ Container App                ca-api-<token>
  ├─ Static Web App                    swa-novimart-<token>
  ├─ Cosmos DB account (Serverless)    cosmos-novimart-<token>
  │    └─ Database "novimart"
  │         ├─ Container "products"   PK: /categoryId
  │         ├─ Container "carts"      PK: /customerId
  │         └─ Container "orders"     PK: /customerId
  ├─ Key Vault (RBAC, purge-protected) kv-novimart-<token>
  ├─ Container Registry (Premium)      acracmeretail<token>
  ├─ Log Analytics workspace           log-novimart-<token>
  ├─ Application Insights              appi-novimart-<token>
  ├─ User-assigned managed identity    id-novimart-api-<token>
...
| Container App returns 500 immediately | Missing managed-identity role on Cosmos | `specfleet review --scope=deploy` — it specifically checks this |
...
You have a fully-functional NoviMart demo running on Azure with:
...
- A repeatable `azd up` / `azd down` lifecycle gated by SpecFleet compliance subagents

To promote to a "prod-grade" environment, run:

```bash
azd env new novimart-prod
# Edit infra/main.parameters.json: minReplicas=2, geo-redundant Cosmos, multi-region SWA
azd up
specfleet review --scope=deploy --env=novimart-prod  # must be PASS, no WARNs
```
