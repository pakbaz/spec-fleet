# NoviMart E-Commerce — Architecture

> **Status:** Living document. Owned by `@novimart/platform-engineering`.
...
NoviMart is a B2C retail commerce sample that demonstrates a GDPR-compliant,
...
(see: `.specfleet/decisions.md`). Persistence is Azure Cosmos DB (NoSQL) with
...
`backend/src/NoviMart.Infrastructure/Auth/EntraAuthExtensions.cs`).
Payments are out of CDE scope by design — the browser is redirected to a
PCI-DSS Level-1 provider, stubbed in this sample by
`StubPaymentProvider` (see: `backend/src/NoviMart.Infrastructure/Payments/StubPaymentProvider.cs`).
...
flowchart LR
    subgraph external["External actors"]
        ...
        Admin([NoviMart Admin<br/>employee])
    end

    subgraph azure["Azure tenant — NoviMart"]
    ...
    API     -->|OTLP| AI
    Shopper -.->|browser redirect — never via NoviMart| Provider
    Provider -.->|opaque token + last4| API
    KV      ===|wraps DEK| Cosmos
```

The dashed line from shopper to provider is the **only** path that carries
PAN/CVV; it never touches NoviMart infrastructure (see: `.specfleet/policies/pci.md:14`).
...
flowchart TB
    ...
    subgraph apps["Application tier (snet-apps)"]
        CAE["Container Apps Environment<br/>cae-novimart-{env}"]
        CA_API["Container App<br/>ca-novimart-{env}-api<br/>(BFF, port 8080)"]
        SWAH["Static Web App<br/>stapp-novimart-{env}<br/>(SPA host)"]
    end

    subgraph data["Data tier (snet-data, private endpoints)"]
        Cosmos[("Cosmos DB<br/>novimart database<br/>6 containers")]
    ...
    subgraph mgmt["Management / observability"]
        LA[/"Log Analytics workspace<br/>log-novimart-{env}"/]
        AI[/"App Insights<br/>appi-novimart-{env}"/]
...
- Customer / admin scheme registration — `EntraAuthExtensions.AddCustomerAuth` (see: `backend/src/NoviMart.Infrastructure/Auth/EntraAuthExtensions.cs:16`) and `AddAdminAuth` (see: `…/EntraAuthExtensions.cs:60`).
- `OwnsResource` policy ensures the JWT `customerId` claim matches the `{customerId}` route parameter (see: `backend/src/NoviMart.Infrastructure/Auth/OwnsResourcePolicy.cs`).
- Cosmos client uses `DefaultAzureCredential` only — connection strings explicitly rejected (see: `backend/src/NoviMart.Infrastructure/Cosmos/CosmosClientFactory.cs:22`).
- `IPaymentProvider` abstracts the redirect contract; `StubPaymentProvider` is the dev/test implementation (see: `backend/src/NoviMart.Infrastructure/Payments/StubPaymentProvider.cs:17`).
- `SensitiveBodyTelemetryFilter` drops bodies on `/payments`, `/checkout`, `/account/payment-methods`, and `/api/me/payment-methods` (see: `backend/src/NoviMart.Infrastructure/Telemetry/SensitiveBodyTelemetryFilter.cs:19`).
...
| `carts`      | `/customerId`   | 30 d (`2592000`) | Confidential   | One customer's cart is read/written together; per-customer PK gives single-partition CRUD. TTL enforces the 30-day retention rule (see: `.specfleet/instruction.md:50`). |
...
> `/aggregateId` per the project spec (see: `.specfleet/project.md:93`,
...
sequenceDiagram
    ...
    rect rgba(255,235,200,0.45)
        Note over U,P: PAN + CVV entered HERE only<br/>NoviMart infra never sees them
        U->>P: card details (HTTPS)
        P-->>U: 302 → NoviMart returnUrl?session=…
    end
...
`.specfleet/policies/pci.md:50-56`).
...
objection (Art. 21) follow the same shape (see: `.specfleet/policies/gdpr.md:32-39`).

## 7. Non-functional requirements

| Concern        | Target                                                                                                    | Source                                              |
| -------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| Availability   | Gold tier — 99.9 % monthly, multi-zone within region                                                      | `.specfleet/project.md:33,100`                            |
| RPO            | 1 h (Cosmos continuous backup → 7-day window)                                                             | `.specfleet/policies/zero-trust.md:90`, `cosmos.bicep:75` |
| RTO            | 4 h                                                                                                       | `.specfleet/policies/zero-trust.md:90`                    |
| Read p99       | ≤ 500 ms on `/api/products`, `/api/categories`                                                            | `.specfleet/instruction.md:43`                            |
| Write p99      | ≤ 1500 ms on `/api/cart`, `/api/orders`                                                                   | `.specfleet/instruction.md:43`                            |
| Bundle (SPA)   | ≤ 200 KB gzipped on initial route                                                                         | `.specfleet/instruction.md:42`                            |
| Throughput     | Container App scales 1–5 replicas on 50 concurrent requests/replica                                       | `infra/modules/containerapps.bicep:173-186`         |
| Coverage       | ≥ 90 % line coverage on changed and adjacent code                                                         | `.specfleet/instruction.md:19`                            |
| Accessibility  | WCAG 2.1 AA on all customer-facing pages                                                                  | `.specfleet/project.md:103`                               |
| Cyclomatic     | ≤ 10 per function                                                                                         | `.specfleet/instruction.md:13`                            |

## 8. Cross-cutting concerns

### 8.1 Logging

- **Backend:** Serilog → App Insights with structured JSON output. Personal data is logged only as `customerId` (a GUID; see: `.specfleet/policies/gdpr.md:60`); never email or name.
...
- **Forbidden bodies:** logging request bodies on `/payments`, `/checkout`, or `/account` is in `instruction.md.forbidden` (see: `.specfleet/instruction.md:75`).
...
- Domain failures use a `Result<T>` envelope (see: `backend/src/NoviMart.Domain/Result.cs`); transport mapping turns failures into 4xx/5xx with stable error codes.
...
| Cosmos DB         | Single region serverless for the sample (see: `cosmos.bicep:56-66`); production uses provisioned throughput with EU/NA region pinning per data residency rules (see: `.specfleet/instruction.md:32`). |
...
1. **Backend:** `dotnet run --project backend/src/NoviMart.Api`. Runs on `http://localhost:5000`. `IPaymentProvider` is bound to `StubPaymentProvider`. `DefaultAzureCredential` falls back to `az login` for AAD-bound services or to the local emulator gate (below).
...
The eight key ADRs from `.specfleet/decisions.md` and policy files:

| #         | Decision                                                                  | Driver                                       | Source                                |
| --------- | ------------------------------------------------------------------------- | -------------------------------------------- | ------------------------------------- |
| ADR-0001  | Cosmos DB (NoSQL) over Azure SQL                                          | Read-heavy, multi-region, schema flexibility | `.specfleet/decisions.md:7-31`              |
| ADR-0002  | API-as-BFF (one .NET API per SPA — no Node tier)                          | Simplicity, single auth audience             | `.specfleet/decisions.md:34-58`             |
| ADR-0003  | Tokenising redirect for payment (full hosted page, not iframe)            | Lowest PCI scope (SAQ-A)                     | `.specfleet/decisions.md:62-89`             |
...
| Implicit  | Region pinning by classification (EU vs NA)                               | GDPR Chapter V transfers                     | `.specfleet/instruction.md:32`              |

When adding a new ADR, append to `.specfleet/decisions.md` and reference the
