...
/// <c>.specfleet/instruction.md</c> forbidden list — no Newtonsoft.Json).
/// </summary>
...
// Read fields used by similarly-named property — avoid the property/type name collision.
        ...
        {
            CustomerId = NoviMart.Domain.ValueObjects.CustomerId.Parse(CustomerId),
            Currency = Currency,
            Items = Items.Select(i => new CartItem
            {
                ItemId = i.ItemId,
                ProductId = NoviMart.Domain.ValueObjects.ProductId.Parse(i.ProductId),
