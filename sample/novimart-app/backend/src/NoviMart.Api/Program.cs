using NoviMart.Api.Configuration;
using NoviMart.Api.Endpoints;
using NoviMart.Infrastructure.Auth;
using NoviMart.Infrastructure.Logging;
...
// ---------------------------------------------------------------------------
// Logging — Serilog with PCI forbidden-field redaction (per .specfleet/policies/pci.md)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
builder.Services.AddNoviMartTelemetry(builder.Configuration);

// ---------------------------------------------------------------------------
// Infrastructure — Cosmos, repositories, payment provider
// ---------------------------------------------------------------------------
builder.Services.AddNoviMartInfrastructure(builder.Configuration);
...
builder.Services.AddCors(options =>
{
    ...
    options.AddPolicy(CorsPolicy, policy =>
    {
        if (origins.Length == 0)
        {
            ...
            // (no wildcards permitted per .specfleet/policies/zero-trust.md §3).
