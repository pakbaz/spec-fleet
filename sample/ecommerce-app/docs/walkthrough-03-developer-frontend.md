...
> **Audience:** A React/TypeScript engineer joining the Acme Retail team and picking up their
> first frontend story under the Enterprise Agents System (EAS).
>
...
>   files.
>
> **What you will learn:** how the **frontend** and **test** subagents collaborate, what an EAS
...
3. [Step 1 — `eas plan`](#step-1--eas-plan-implement-cart-drawer-story-2)
4. [Step 2 — Review the plan (human gate)](#step-2--review-the-plan-human-gate)
5. [Step 3 — `eas implement --task=feature-cart-api-client`](#step-3--feature-cart-api-client)
6. [Step 4 — `eas implement --task=feature-cart-persistence`](#step-4--feature-cart-persistence)
7. [Step 5 — `eas implement --task=feature-cart-drawer-ui`](#step-5--feature-cart-drawer-ui)
8. [Step 6 — `eas implement --task=tests-cart-drawer`](#step-6--tests-cart-drawer)
9. [Step 7 — `eas review`](#step-7--eas-review)
...
# Repo setup
cd sample/ecommerce-app/frontend
npm install
...
```bash
cd sample/ecommerce-app/backend
ASPNETCORE_ENVIRONMENT=Development dotnet run --project src/Acme.Retail.Api
```

Verify EAS is healthy:

```bash
eas doctor
# ✓ .eas/instruction.md present (sha 4e7a…b1)
...
The story is in `.eas/plans/story-2-cart.md` (committed by the architect agent). Highlights:
...
## Step 1 — `eas plan "implement cart drawer (story 2)"`

```bash
eas plan "implement cart drawer (story 2)"
```

Excerpt of orchestrator output (full transcript: `.eas/audit/2024-04-27_plan_story-2.jsonl`):
...
[orchestrator] writing .eas/plans/story-2.plan.md
...
Open `.eas/plans/story-2.plan.md`, read the dependency DAG, and approve:

```bash
eas resume --gate=plan_approved --comment "lgtm, please proceed in order"
```

The orchestrator records the approval in the audit log and resumes:

```jsonl
{"ts":"…","event":"gate_resumed","gate":"plan_approved","actor":"alice@acme.example",
...
```bash
eas implement --task=feature-cart-api-client
```

The frontend subagent receives a brief that contains:

- `.eas/charters/dev.charter.md` and `subagents/dev/frontend.charter.md`
- The Acme house style: `src/features/<area>/api.ts` co-located, **Zod** schemas in `schemas.ts`
- Read-only access to `backend/src/Acme.Retail.Contracts/Cart*.cs` to mirror DTO shapes
- A token budget reservation of **14,000** tokens

Files written:

```text
sample/ecommerce-app/frontend/src/features/cart/schemas.ts
sample/ecommerce-app/frontend/src/features/cart/api.ts
```
...
The `onPostToolUse` hook ran `eas charter validate --file frontend.charter.md` and the secret
...
```bash
eas implement --task=feature-cart-persistence
```

The subagent writes `useCartPersistence.ts` — a small hook that:

- Reads/writes the cart to `localStorage` under key `acme.cart.v1` with a JSON `version` field.
...
```bash
eas implement --task=feature-cart-drawer-ui
```
...
```bash
eas implement --task=tests-cart-drawer
```
...
that **only** `acme.cart.v1` is present and contains no PII.
