# Walkthrough 1 — Admin setup: bringing EAS online for Acme Retail

> **Audience:** Platform / Security administrator standing up the Enterprise
> Agents System (EAS) for the **Acme Retail e-commerce** repo.
> **Estimated time:** 60–90 minutes (most of it reading and reviewing).
> **Outcome:** A reviewed, signed-off `.eas/` directory in `main`, locked down
> by CODEOWNERS, validated by `eas doctor`, and mirrored into
> `.github/agents/` for graceful degradation.

This walkthrough is the **first** of four. It targets a single human role: the
admin who is responsible for encoding Acme Retail's engineering, security, and
...
| Tool | Version | Why |
|---|---|---|
| Node.js | ≥ 20.0.0 | Runs the `eas` CLI and the GitHub Copilot SDK |
...
```bash
git clone https://github.com/acme-retail/enterprise-agents-system.git
cd enterprise-agents-system
npm install
npm run build
npm link            # exposes `eas` on $PATH for the rest of this guide
...
Usage: eas [options] [command]

Enterprise Agents System — autonomous ALM on the GitHub Copilot SDK + CLI.

Commands:
  init                 Bootstrap .eas/ and run the architect interview
  onboard              Brownfield: detect stack, draft project.md
  plan <goal>          Decompose a goal into role-agent tasks
  implement [opts]     Execute the next ready task (or --all)
  review               Compliance + Architect re-review the git diff
  charter <subcmd>     new | list | validate | sync-mirror
  status               Charters, plans, and audit summary
  audit [--tail]       Stream audit events
  doctor               Validate .eas/ integrity and environment
```
...
> **Note:** If the sample's `.eas/` directory already exists (it does in this
> repo), treat the walkthrough as a guided code-review tour. Every step has a
> "what's already on disk" pointer so you can compare what `eas init` would
> have produced against the reviewed, hand-tuned version that is committed.

---

## Step 1 — `eas init`: the architect interview

`eas init` is a guided interview hosted by the **Architect role agent** (see
`.eas/charters/architect.charter.md`). It writes three files:
`.eas/instruction.md` (corporate standards stub), `.eas/project.md` (the
project spec), and `.eas/decisions.md` (an empty ADR log).
...
EAS init — guided architect interview
Session: arc-7f3c (charter: architect, budget: 80,000 tokens)
Audit:   .eas/audit/arc-7f3c.jsonl

? What is the project name?  Acme Retail E-Commerce
? Mode (greenfield / brownfield / modernization)?  greenfield
? One-line description?
  B2C e-commerce for Acme Retail — catalog, cart, checkout via tokenising
...
✓ Wrote .eas/instruction.md   (template — review and harden)
✓ Wrote .eas/project.md
✓ Wrote .eas/decisions.md
✓ Wrote 7 role charters under .eas/charters/
✓ Wrote 18 subagent charters under .eas/charters/subagents/
✓ Wrote .eas/policies/secrets.json (default redaction patterns)
✓ Wrote .eas/mcp/.gitkeep      (scoped MCP manifests live here)
✓ Wrote .eas/audit/.gitkeep    (append-only JSONL log)
✓ Wrote .eas/CODEOWNERS.example

Next steps:
  1. Harden .eas/instruction.md (it is your corporate contract).
  2. Run: eas charter validate
  3. Run: eas doctor
  4. PR `.eas/` into main with security + compliance reviewers.
```
...
> `.eas/audit/arc-7f3c.jsonl`. The interview is the only EAS flow that is
> allowed to write `.eas/instruction.md` — and only when the file does not
...
`.eas/instruction.md` is the single, versioned, signed-off statement of "what
Acme Retail considers a shippable change." Every subsequent agent session
...
The committed version is at `sample/ecommerce-app/.eas/instruction.md`. Open
...
owners:
  - "platform-engineering@acme-retail.example"
  - "security-leads@acme-retail.example"
  - "compliance@acme-retail.example"
policies:
  coding:        [...]
  security:      [...]
  compliance:    [...]
...
| Rule | Maps to | Why |
|---|---|---|
| No secrets in source. | NIST IA-5 | Secret-scanning subagent + `.eas/policies/secrets.json` redactor. |
...
- **Encryption at rest with CMK for restricted data** — GDPR Art. 32(1)(a)
  ("appropriate technical measures"). CMK gives Acme a documented kill switch.
...
Acme Retail's posture is **out of CDE scope by design**. The two relevant
clauses:

- "Cardholder data MUST NEVER be stored, logged, or transmitted by Acme Retail
...
> `eas charter validate` (Step 5), the runtime parses this YAML against its
> Zod schema (`src/schema/`) and refuses to start any session if it is
> malformed. When you run `eas review` later, the Compliance role agent
...
`instruction.md` is the contract. The `.eas/policies/` directory carries
...
files are committed for Acme Retail:
...
```bash
eas charter new compliance-gdpr --parent compliance
```

```text
# Simulated output — actual command emits similar
✓ Created .eas/charters/subagents/compliance/gdpr.charter.md
...
Edit the prompt body, then run: eas charter validate
```

Open `.eas/charters/subagents/compliance/gdpr.charter.md`. The frontmatter
...
> **What just happened?**
>
> `eas charter new` only scaffolded a file. The runtime does **not** yet
> trust this charter — `eas charter validate` (Step 5) is what binds it
...
## Step 4 — Lock down `.eas/instruction.md`
...
```bash
mkdir -p .github
cp .eas/CODEOWNERS.example .github/CODEOWNERS
```

The Acme Retail rules:

```text
# Default
*                                @acme-retail/platform-engineering

# Immutable EAS artifacts — restricted to security and compliance
.eas/instruction.md              @acme-retail/security-leads @acme-retail/compliance
.eas/policies/                   @acme-retail/security-leads @acme-retail/compliance
.eas/charters/                   @acme-retail/platform-engineering
.eas/charters/subagents/compliance/   @acme-retail/security-leads @acme-retail/compliance

# Backend / Frontend / IaC
backend/                         @acme-retail/backend-team
backend/src/Acme.Retail.Infrastructure/Payments/   @acme-retail/payments-team @acme-retail/compliance
frontend/                        @acme-retail/frontend-team
infra/                           @acme-retail/platform-engineering @acme-retail/security-leads
.github/workflows/               @acme-retail/platform-engineering
```

Combine with a **branch protection rule** on `main` that requires CODEOWNERS
review. The committed `sample/ecommerce-app/.github/CODEOWNERS` already
encodes this.
...
`eas implement` is running unattended. It is configured in
`.eas/charters/<role>.charter.md` (or globally) and lives in source at
`src/hooks/`. The behaviour:

```text
# Simulated output — actual hook fires inline during a session
[hook] onPreToolUse  charter=dev/backend  tool=write  path=.eas/instruction.md
...
[audit] {"ts":"2025-01-20T11:04:22Z","sessionId":"dev-9c1a","event":"policy.block",
         "rule":"immutable","path":".eas/instruction.md"}
```
...
> merge without `@acme/security-leads` + `@acme/compliance` approval, and
> the EAS runtime will refuse the write attempt before it ever reaches a
> commit. Either gate alone is a single point of failure.

---

## Step 5 — `eas charter validate`

```bash
eas charter validate
```

```text
# Simulated output — actual command would emit similar
EAS charter validate — sample/ecommerce-app/.eas/charters/
...
Charter graph
  ...
  - All MCP scopes resolve to a manifest in .eas/mcp/.  ✓
  - No orphan parents.  ✓
  - No cycles.  ✓
...
`eas doctor` is the green-light check before merging:
...
```text
# Simulated output — actual command would emit similar
EAS doctor — sample/ecommerce-app
...
[.eas integrity]
  ...
  ✓ 29 charters loaded (see `eas charter validate`)
...
[telemetry]
  ✓ Audit dir: .eas/audit/  (mode 0644, append-only)
  ✓ Last 7 sessions complete (no orphan locks)

ALL GREEN — safe to commit and PR.
```

If `doctor` finds anything yellow (e.g. CODEOWNERS missing for
...
```bash
git checkout -b admin/eas-bootstrap
git add .eas/ .github/CODEOWNERS docs/walkthrough-01-admin-setup.md
git commit -m "chore(eas): bootstrap .eas/ for Acme Retail
...
git push -u origin admin/eas-bootstrap
gh pr create --base main --fill --reviewer acme-retail/security-leads,acme-retail/compliance
```

PR description (recommended template):

```markdown
## EAS bootstrap — Acme Retail

This PR establishes the Enterprise Agents System (EAS) governance layer.

### What this PR does
- Adds `.eas/` with the immutable engineering contract (`instruction.md`).
- Defines the 29-agent hierarchy (7 role + 22 subagent charters).
- Wires GDPR / PCI scope-reduction / Zero-Trust policies into the
  Compliance role.
- Adds CODEOWNERS so future changes to `.eas/instruction.md` and
  `.eas/policies/` require Security + Compliance approval.
...
- [ ] `eas charter validate` is green in CI.
- [ ] `eas doctor` is green in CI.
```

CODEOWNERS rules will auto-request review from
`@acme-retail/security-leads` and `@acme-retail/compliance`. The PR cannot
merge without both approvals.

> **What just happened?**
>
> The `.eas/` directory is now under the same change-management discipline
...
## Step 8 — `eas charter sync-mirror`: graceful degradation

Not every engineer will have the `eas` CLI installed on day one. To make
...
```bash
eas charter sync-mirror
```

```text
# Simulated output — actual command emits similar
EAS charter sync-mirror — writing graceful-degradation copies
...
best-effort UX so that engineers without the EAS runtime still load the
correct prompt. Only the EAS runtime is policy-bearing.
```
...
git commit -m "chore(eas): mirror charters to .github/agents for fallback UX

Co-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"
```

> **Warning:** The mirror is **read-only convenience**. If a developer edits
> `.github/agents/dev.agent.md` directly, `eas charter validate` will fail
> the next CI run because the source of truth (`.eas/charters/dev.charter.md`)
> is unchanged and the mirror is now out of sync. CI re-runs `sync-mirror`
> as a check on every PR.
...
1. Open a PR adding the rule under the appropriate `policies.*` array in
   `.eas/instruction.md`.
2. CODEOWNERS auto-requests `@acme/security-leads` + `@acme/compliance`.
3. Append a one-paragraph entry to `.eas/decisions.md` explaining
...
5. After merge, the next agent session loads the new rule. The Compliance
   agent will start enforcing it on the next `eas review` or PR.

### Retire a charter

1. Run `eas charter list` to confirm no other charter declares it as
   `parent`. If any do, retire those first (or re-parent them).
2. Open a PR deleting the charter file. CI will re-run
   `eas charter validate` and confirm no orphan parents.
...
4. Append an entry to `.eas/decisions.md`.
...
2. CI runs `eas review --dry-run` against the last 14 days of merged PRs to
   show the false-positive rate the new rule would have caused.
3. Post the report in the PR. Open for **2 weeks** of org-wide comment.
...
│   ├── plans/                      # populated by `eas plan`
│   ├── checkpoints/                # populated by `eas implement`
...
- **Walkthrough 2 — Developer flow.** A backend engineer runs `eas plan` →
  `eas implement` to add a new endpoint, hits a compliance gate, fixes,
  ships.
- **Walkthrough 3 — Reviewer flow.** A compliance reviewer uses
  `eas review` + `eas audit` to pre-screen a complex PR.
- **Walkthrough 4 — DevOps deployment.** First-time deploy of the full
  Acme Retail stack to Azure with `azd up`, including the EAS-driven
