# Zero Trust Controls — Acme Retail

> **Scope:** Maps Acme Retail's implementation choices to Microsoft's Zero Trust pillars and
> shows where in the codebase or infrastructure each control is enforced. This is the reference
> document for the `compliance/zero-trust` EAS subagent.
>
...
|--------|-----------------|----------|
| Identity | Entra ID (admin) + Entra External ID (customer); MFA enforced via Conditional Access; JWT validation on every API call | `Acme.Retail.Infrastructure/Auth/*` |
...
| Application | API authorization gates (`RequireCustomer`, `RequireAdmin`, `OwnsResource`); typed contracts; antiforgery on state-changing routes | `Acme.Retail.Infrastructure/Auth/AuthPolicies.cs`, `Program.cs` |
...
| Visibility | OTel traces → App Insights; Serilog → Log Analytics; EAS audit log → file → LA in prod; alerts on anomalous auth failures | `Program.cs`, `.eas/audit/` |
| Automation | EAS subagents enforce policies on every diff; `azd` hooks gate provisioning; CI runs `eas review --strict` | `.eas/charters/`, `.github/workflows/security.yml` |
...
The EAS `compliance/zero-trust.charter.md` records this as a **prerequisite** for any prod
...
after `azd up` completes — the EAS post-provision hook removes `Secrets Officer` from the
...
The EAS `compliance/zero-trust` subagent runs an **endpoint coverage audit** during
`eas review`: it walks every route registration in `Program.cs` and asserts that every
...
- **Audit log**: every EAS tool invocation, every gate approval, every rule violation is
  written as JSONL to `.eas/audit/` and shipped to Log Analytics by the production deploy.
...
| Source code tamper | Branch protection on `main`; required reviews; CODEOWNERS protects `.eas/instruction.md` |
...
## 7. EAS subagent enforcement summary
...
| `onPostToolUse` (write) | Re-run `eas charter validate` with full charter set |
| `eas review --scope=code` | Endpoint coverage audit (every non-anonymous route has a policy) |
| `eas review --scope=infra` | Bicep diff: no `publicNetworkAccess: Enabled`, no `enabledForDeployment: true`, no SAS-token usage |
| `eas review --scope=deploy` | Verify runtime resource posture (the deploy walkthrough §5 list) |
...
- Acme Retail policy file: `.eas/policies/zero-trust.md`
