# Acme Retail — Enterprise Agents System Sample

> A complete, runnable demonstration of the **Enterprise Agents System (EAS)** governing a
...
> pipeline. Wrapped end-to-end with EAS charters that enforce GDPR, PCI scope reduction, and
> Zero Trust controls.
...
├── .eas/                           # The corporate spec — admin-owned, immutable in CI
│   ├── instruction.md              # Acme Retail standards (the "constitution")
...
│   ├── plans/                      # Story plans authored by `eas plan`
│   ├── audit/                      # Tool-call audit log (JSONL)
│   ├── checkpoints/                # Compacted session memory
│   └── skills/                     # Lazy-loaded procedures
│
├── backend/                        # .NET 10 BFF API
│   ├── src/Acme.Retail.Api         # Minimal API host (Program.cs, endpoints)
│   ├── src/Acme.Retail.Domain      # Entities, value objects, Result<T>, specs
│   ├── src/Acme.Retail.Infrastructure  # Cosmos repos, Entra auth, payment stub
│   ├── src/Acme.Retail.Contracts   # Request/response DTOs
│   └── tests/Acme.Retail.UnitTests # xUnit + FluentAssertions + NSubstitute
│
...
- **EAS CLI** — install from the repo root: `npm i -g .` (this is the meta-runtime; sample
...
|---|-------------|----------|------|
| 1 | [Admin sets up the EAS spec](docs/walkthrough-01-admin-setup.md) | Platform admin | 30 min |
...
Each walkthrough shows the **real EAS CLI commands**, the **simulated agent transcripts** for
...
Every one of these is **enforced** by an EAS subagent charter — see `.eas/charters/` and the
walkthrough sections that show the subagent firing.

---

## Running the EAS commands against this sample

```bash
cd sample/ecommerce-app

# Validate the .eas/ directory is internally consistent
eas doctor
...
For `eas plan` / `eas implement` / `eas review` — see the relevant walkthrough; those
...
| Area | Status |
|------|--------|
| `.eas/` — charters, policies, sample instruction & project | ✅ Authored |
...
2. **Inspect the charters.** Open `.eas/charters/dev.charter.md` and any subagent under
   `.eas/charters/subagents/` — these are the heart of the system.
...
4. **Write your own story.** Add a third user story (e.g., "wishlist") and observe how
   `eas plan` decomposes it across the same agent hierarchy.
