...
policies:
  ...
  compliance:
    ...
    - "Cardholder data (PAN, CVV, full track) MUST NEVER be stored, logged, or transmitted by Acme Retail systems. All payment flows MUST use a tokenising redirect/iframe to a PCI-DSS Level-1 provider."
    - "PCI-DSS scope reduction is mandatory: the Acme Retail backend is OUT of CDE scope by design. Any PR that introduces PAN or SAD into our systems MUST be blocked."
...
contacts:
  security: "secops@acme-retail.example"
  compliance: "compliance@acme-retail.example"
  platform: "platform@acme-retail.example"
  privacy: "dpo@acme-retail.example"
---

# Acme Retail — Engineering Standards
...
> **Mutability:** This file is **immutable** at runtime. EAS will refuse any
> agent-driven write to it. Changes require a PR with approvals from
> `@acme-retail/security-leads` and `@acme-retail/compliance` (enforced by
> CODEOWNERS).

## How to read this file

Every EAS agent reads `policies.*` arrays at session start and treats each
...
Acme Retail processes EU and NA customer data (GDPR applies). Acme Retail is
...
3. After merge, all new EAS sessions pick up the change on next invocation.
4. Append a corresponding entry to `.eas/decisions.md`.
