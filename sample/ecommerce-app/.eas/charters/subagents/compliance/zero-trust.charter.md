...
displayName: Compliance — Zero Trust
role: compliance
tier: subagent
parent: compliance
...
1. `.eas/instruction.md` → `policies.security`, `policies.compliance`
2. `.eas/project.md` → `nfr.securityTier` (`elevated` or `regulated` triggers stricter checks)
3. `.eas/policies/zero-trust.md` (project Zero Trust controls map)
