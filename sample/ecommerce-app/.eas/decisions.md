# Acme Retail E-Commerce — Decisions Log

Append-only ADR log written by EAS agents and engineers. Newest entries at the bottom.
...
- **kind**: decision
- **status**: accepted
- **refs**: [`.eas/instruction.md`, `.eas/policies/pci.md`, `docs/pci-scope-boundary.md`]

### Context
Acme Retail must not enter the PCI-DSS CDE. Two options surveyed:
(a) tokenising redirect to a hosted payment page;
(b) embedded iframe with hosted fields.

### Decision
**Option (a)** — full redirect. The browser navigates from the Acme checkout
page to the payment provider's hosted page; on completion, the provider
redirects back with an opaque token + status; Acme never sees the PAN or CVV.
