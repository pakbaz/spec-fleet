# PCI-DSS Policy — Acme Retail E-Commerce

> **Project policy file** — applied by `compliance/pci` charter on every review.
> Owner: `compliance@acme-retail.example`.
> **Scope strategy: REDUCTION.** Acme Retail systems are out of CDE scope by
...
|---|---|---|
| Browser → Acme SPA | **OUT** | Renders payment-button only; no PAN entry on Acme pages |
| Acme React SPA | **OUT** | Triggers redirect to provider; never receives PAN/CVV |
| Acme .NET API | **OUT** | Receives token + status only; PAN/CVV never in request bodies |
| Acme Cosmos DB | **OUT** | Stores `paymentToken`, `last4`, `bin`, `cardBrand` (allowable per Req. 3) |
| Acme audit log | **OUT** | Logs token only; never PAN |
| Acme App Insights | **OUT** | Request bodies on `/payments` and `/checkout` are dropped (telemetry filter) |
| Payment provider hosted page | **In their CDE** | Acme has no access |
| Payment provider tokenisation | **In their CDE** | Acme has no access |
...
4. Introduces a code path that calls a payment provider's *non-tokenising* API directly from Acme code
...
- Telemetry filter `dropPaymentBodies` is registered in `Acme.Retail.Api` startup
...
The sample uses `Acme.Retail.Infrastructure.Payments.StubPaymentProvider`,
...
- Penetration test of Acme perimeter (not CDE — perimeter as required by SAQ-A v4 r2 evolving criteria)
