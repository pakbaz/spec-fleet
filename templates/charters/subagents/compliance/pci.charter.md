---
name: compliance/pci
displayName: Compliance — PCI-DSS
role: compliance
tier: subagent
parent: compliance
...
1. `.eas/instruction.md` → `policies.compliance`
2. `.eas/project.md` → `complianceScope` (must include `pci-dss` for this charter to apply)
3. `.eas/policies/pci.md` (project policy, especially the **PCI scope boundary** — what is in/out of scope)
...
- No hard-coded keys/tokens (rely on EAS secret-redaction hook + Key Vault refs).
