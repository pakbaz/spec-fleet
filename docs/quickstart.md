...
`eas onboard` runs a stack heuristic (package.json / pyproject.toml /
...
eas audit --tail                  # stream JSONL events live
eas audit --since 1h --agent dev/backend
eas doctor                        # validate .eas/ integrity (charter graph, MCP refs, caps)
```
...
## 8. Customize charters

Charters live in `.eas/charters/<role>/<name>.charter.md`. Each is YAML
frontmatter + markdown body:

```bash
eas charter list
eas charter new dev/graphql --role dev --tier subagent --parent dev
$EDITOR .eas/charters/dev/graphql.charter.md
eas charter validate              # check graph integrity before commit
```

The mirror to `.github/agents/` is regenerated automatically on the next
`eas` invocation, so plain `copilot` users inherit the new agent.
...
jobs:
  review:
    ...
    steps:
      ...
      - run: npm i -g enterprise-agents-system
      - run: eas doctor
      - run: eas review
        env:
...
| `Charter "x" references missing parent "y"` | Run `eas charter validate`; either add `y` or remove the reference |
...
| Agent runs but produces nothing useful | Inspect `.eas/audit/<sessionId>.jsonl` — every prompt and tool call is there |
